...
import {
  User, FileText, Trash2, Save, Activity, Clipboard,
  Beaker, FileSpreadsheet, CheckCircle, AlertCircle, Clock, Search,
  ChevronDown, Calendar, PlusCircle
} from "lucide-react";
...
export default function PatientRecord() {
  ...
  // --- NEW: Catalog Auto-Suggestion State Variables ---
  ...
  // ---- Load Doctor Identity and Specialization ----
  useEffect(() => {
  ...
  // ---- Fetch Clinical Catalogs (Medicines & Labs) ----
  useEffect(() => {
    ...
    const fetchCatalogs = async () => {
      try {
        ...
        console.error("Failed to sync clinical ecosystem medical catalogs:", err);
      }
    };
    fetchCatalogs();
  ...
  // --- NEW: Medicine Suggestions Selection ---
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (isCreatingNew) {
        ...
        setSelectedRecord({ ...created, appointmentDate: selectedRecord.appointmentDate, appointmentTime: selectedRecord.appointmentTime, appointmentReason: selectedRecord.appointmentReason });
      } else {
        ...
        setSelectedRecord({ ...updated, appointmentDate: selectedRecord.appointmentDate, appointmentTime: selectedRecord.appointmentTime, appointmentReason: selectedRecord.appointmentReason });
      }
    } catch (err) {
  ...
  const handleDeleteRecord = async () => {
    ...
    try {
      ...
      await axios.delete(
        `${API_BASE_URL}/api/v1/doctors/e-prescription/${selectedRecord._id}?doctorEmail=${encodeURIComponent(doctorEmail)}`
      );
    ...
    <div className="pr-console-root pr-fade-in">
      <div className="pr-console-header">
        <div className="pr-header-meta">
          ...
          <p>Review, update parameters, initialize entries or delete historic clinical records assigned to patients.</p>
        </div>
      </div>
      ...
      <div className="pr-patient-dropdown-bar">
        <label className="pr-dropdown-label"><User size={16} /> Select Patient Chart</label>
      ...
      <div className="pr-split-workspace">
        <div className="pr-patients-sidebar-card">
          <div className="pr-search-box-wrapper">
            ...
            <input type="text" placeholder="Search appointments history logs..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>

          <div className="pr-sidebar-title"><Clipboard size={16} /> Patient Consultation Timeline ({filteredTimeline.length})</div>
              ...
              {filteredTimeline.length > 0 ? (
                filteredTimeline.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`pr-patient-node ${isCurrentSelection ? 'active' : ''} ${!hasPrescription ? 'node-pending-action' : ''}`}
                      onClick={() => handleSelectTimelineItem(item)}
                    >
                      <div className="pr-patient-meta">
                        <strong>{hasPrescription ? item.prescription.prescriptionName || "Untitled Prescription" : "Missing E-Prescription"}</strong>
                        <span className={`pr-diag-tag ${!hasPrescription ? 'tag-missing' : ''}`}>
                          {hasPrescription ? item.prescription.diagnosis || "Diagnosis Missing" : "No record entry"}
                        </span>
                        <span><Calendar size={12} /> {formatDate(item.appointmentDate)}{item.appointmentTime ? ` · ${item.appointmentTime}` : ""}</span>
                      </div>
                    </div>
                  );
                })
                ...
                <div className="pr-empty-fallback">
                  <Activity size={32} />
                  <p>No historical clinical records matched search metrics configuration.</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="pr-form-canvas-card">
          {!selectedRecord ? (
            <div className="pr-placeholder-overlay">
              ...
              <p>Select a consultation timeline slot from the left side ledger panel to view details, configure edits, or sign missing prescriptions.</p>
            </div>
          ) : (
            <form onSubmit={handleFormSubmit} className="pr-intake-form pr-fade-in">
              <div className="pr-context-strip">
                <div><strong>Patient Name:</strong> {historyData?.patient?.name || "Unknown"}</div>
                <div><strong>Identifier Email:</strong> {selectedRecord.patientEmail}</div>
                <div><strong>Consult Status:</strong> {isCreatingNew ? "Prescription Draft Mode" : "Historical Record Archival"}</div>
                <div className="pr-full-strip"><strong>Visit Slot Date:</strong> {formatDate(selectedRecord.appointmentDate)}{selectedRecord.appointmentTime ? ` (${selectedRecord.appointmentTime})` : ""}</div>
                <div className="pr-full-strip"><strong>Chief Complaint Reason:</strong> {selectedRecord.appointmentReason || "Routine check-up track consult."}</div>
              </div>

              <div className="pr-form-grid">
                <div className="pr-input-block">
                  <label>Prescription Sheet Title</label>
                  <input type="text" name="prescriptionName" placeholder="e.g., Post-Viral Cough Therapy Protocol" value={selectedRecord.prescriptionName || ""} onChange={handleFieldChange} />
                </div>
                <div className="pr-input-block">
                  <label>Clinical Primary Diagnosis</label>
                  <input type="text" name="diagnosis" placeholder="e.g., Acute Post-Infectious Bronchitis" value={selectedRecord.diagnosis || ""} onChange={handleFieldChange} />
                </div>
              </div>

              <div className="pr-section-block">
                <div className="pr-section-header"><FileSpreadsheet size={16} /> Pharmaceutical Dosing Plan</div>
                  ...
                  return (
                    <div key={idx} className="pr-med-row">
                      <div className="pr-input-block flex-3 relative-container">
                        ...
                        <input type="text" value={med.medicine || ""} required placeholder="Type to filter medical catalogs..." onChange={(e) => handleMedTyping(idx, e.target.value)} onFocus={() => setMedQueries(p => ({ ...p, [idx]: true }))} onBlur={() => setTimeout(() => setMedQueries(p => ({ ...p, [idx]: false })), 200)} />
                      ...
                      <div className="pr-input-block flex-2">
                        ...
                        <input type="text" value={med.dosage || ""} required placeholder="e.g., 1 tablet daily" onChange={(e) => handleMedicineChange(idx, "dosage", e.target.value)} />
                      </div>
                      <div className="pr-input-block flex-1">
                        <label>Days</label>
                        <input type="number" value={med.days || ""} required placeholder="5" onChange={(e) => handleMedicineChange(idx, "days", e.target.value)} />
                      </div>
                      <button type="button" className="pr-remove-btn" onClick={() => removeMedicineRow(idx)}><Trash2 size={16} /></button>
                    </div>
                  );
                })}
                <button type="button" className="pr-add-row-btn" onClick={addMedicineRow}>+ Append New Medicine Line</button>
              </div>

              <div className="pr-section-block">
                <div className="pr-section-header"><Beaker size={16} /> Laboratory Evaluation Requests</div>
                  ...
                  return (
                    <div key={idx} className="pr-lab-row">
                      <div style={{ flex: 1 }} className="relative-container">
                        <input type="text" value={lab || ""} required placeholder="Search diagnostic panels..." onChange={(e) => handleLabTyping(idx, e.target.value)} onFocus={() => setLabQueries(p => ({ ...p, [idx]: true }))} onBlur={() => setTimeout(() => setLabQueries(p => ({ ...p, [idx]: false })), 200)} />
                      ...
                      <button type="button" className="pr-remove-btn" onClick={() => removeLabRow(idx)}><Trash2 size={16} /></button>
                    </div>
                  );
                })}
                <button type="button" className="pr-add-row-btn" onClick={addLabRow}>+ Add Additional Lab Panel Request</button>
              </div>

              <div className="pr-section-block">
                <div className="pr-section-header"><FileText size={16} /> Directives & Outcomes</div>
                <div className="pr-input-block">
                  <label>Supplementary Care Directives</label>
                  <textarea name="notes" placeholder="Lifestyle advice, follow-up constraints..." value={selectedRecord.notes || ""} onChange={handleFieldChange} />
                </div>
                <div className="pr-input-block" style={{ marginTop: "1rem" }}>
                  <label>Post-Consultation Summary Result <span className="pr-req">*</span></label>
                  <textarea name="result" required placeholder="Clinical summary status findings outcome data..." value={selectedRecord.result || ""} onChange={handleFieldChange} />
                </div>
              </div>
              ...
              <div className="pr-action-button-group">
                {isCreatingNew ? (
                  <button type="submit" disabled={submitting} className="pr-btn-save btn-create-accent">
                    <PlusCircle size={16} /> {submitting ? "Signing Record..." : "Authorize & Sign E-Prescription"}
                  </button>
                ) : (
                  <>
                    <button type="submit" disabled={submitting} className="pr-btn-save">
                      <Save size={16} /> {submitting ? "Saving..." : "Save Historical Changes"}
                    </button>
