...
export default function Receptionistappointmentmanage() {
  ...
  useEffect(() => {
    ...
    const fetchDoctors = async () => {
      try {
        ...
        console.error("Failed to pull matching doctors.", err);
        setDoctorsFromDb([]);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchDoctorMetadataAndSlots = async () => {
      try {
        ...
        console.error("Failed to query doctor schedule.", err);
        setDoctorAvailability({ defaultWeeklySlots: [], customDayOverrides: {}, activeBookings: [] });
        setSelectedDoctorProfile(null);
      } finally {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || "Unable to save appointment request.");
    }
  };

  const handleDirectConfirmAction = async (appointmentId) => {
    try {
      setActionLoading((prev) => ({ ...prev, [appointmentId]: true }));
      await axios.patch(`${API_BASE_URL}/api/v1/receptionist/appointments/${appointmentId}/action`, {
        action: "confirm"
      });
  ...
  const handleTriggerReschedule = (apt) => {
    ...
    setPatientData({
      ...
      email: apt.patientEmail || apt.patient?.email || ""
    });
  ...
  const categorizedLedgers = useMemo(() => {
    ...
    appointments.forEach((apt) => {
      const normalizedApt = {
        ...
        patientName:
          apt.patientName ||
          `${apt.patientFirstName || apt.patient?.firstName || ""} ${apt.patientLastName || apt.patient?.lastName || ""}`.trim() ||
          "Patient",
      ...
      const isPast = isPastTimeSlot(normalizedApt.date, normalizedApt.time);

      if (normalizedApt.status === "cancelled" || normalizedApt.status === "rejected") {
        lists.cancelled.push(normalizedApt);
      } else if (normalizedApt.status === "pending" && isPast) {
        lists.cancelled.push(normalizedApt);
      } else if (normalizedApt.status === "confirmed" && isPast) {
        lists.visited.push(normalizedApt);
      } else if (normalizedApt.status === "confirmed") {
        lists.confirmed.push(normalizedApt);
      } else {
        lists.pending.push(normalizedApt);
      }
    });
  ...
  const renderAppointmentCardList = (list) => {
    return list.map((apt) => {
      ...
      const isProcessingAction = !!actionLoading[appointmentId];
              ...
              <div className="accordion-actions-strip">
                {apt.status === "pending" && !isPastTimeline && (
                  <button
                    ...
                    onClick={() => handleDirectConfirmAction(appointmentId)}
                  >
                ...
                {!isTotalLocked && (
                  <>
                    ...
                    <button type="button" className="action-btn btn-cancel" onClick={() => handleCancelAppointment(appointmentId)}>
                      <XCircle size={14} />
                      Cancel Session
                    </button>
                  </>
          ...
          <form className="booking-form-panel" onSubmit={handleFormSubmit}>
            ...
            <div className="disabled-demographics-grid">
              ...
              <div className="input-node full-width">
                <label>Patient Registered Email</label>
                <input
                  type="email"
                  name="email"
                  ...
                  <div className="slots-flex-box">
                    {getSlotsForDate(bookingForm.selectedDate).length > 0 ? (
                      getSlotsForDate(bookingForm.selectedDate).map((slot) => {
                        const isSameEditingSlot =
                          editingAppointmentId &&
                          appointments.some((apt) => getAppointmentId(apt) === editingAppointmentId && normalizeDate(apt.date || apt.appointment_date) === bookingForm.selectedDate && (apt.time || apt.time_slot) === slot);

                        const isSlotTaken =
                          !isSameEditingSlot &&
                          doctorAvailability.activeBookings?.some((booking) => booking.date === bookingForm.selectedDate && booking.time === slot);
            ...
            <div className="wizard-action-footer">
              <button type="button" className="btn-cancel" onClick={closeBookingForm}>
                Cancel
              </button>
              ...
              <div className="interactive-scheduling-stack">
                <div className="calendar-widget-wrapper">
                  <div className="cal-navigation-row">
                    <button type="button" onClick={() => setCurrentCalDate(new Date(currentCalDate.getFullYear(), currentCalDate.getMonth() - 1, 1))}>
                      <ChevronLeft size={16} />
                    </button>
                  ...
                  <div className="cal-matrix-grid">
                    ...
                    {Array.from({ length: daysInMonth(currentCalDate.getFullYear(), currentCalDate.getMonth()) }).map((_, index) => {
                      const day = index + 1;
                      ...
                      const isHoliday =
                        isWeekend &&
                        (!doctorAvailability.customDayOverrides?.[loopDateStr] ||
                          doctorAvailability.customDayOverrides[loopDateStr].length === 0);
                ...
                {selectedDoctorProfile && (
                  <div className="dr-embedded-profile-accordion animate-slide-up">
                    <div className="accordion-heading">
                      <Award size={14} />
                      Professional Practitioner Summary
                    </div>
